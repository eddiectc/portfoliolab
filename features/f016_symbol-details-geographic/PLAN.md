...
- [x] Verify each spec scenario against actual behavior
- [x] Check edge cases from the spec (partial data, no data, name inconsistencies stored as-is)
...
**Verification:** All tests pass (except pre-existing flaky `TestSymbolDetails_CreateAndEnrich`), all spec scenarios validated, no unresolved issues.
