...
- `FxConverter` was left out of the `MarketDataService` abstraction — it has its own complex cache-fetch-fallback chain for FX rates that's a separate concern. It continues to depend on `MarketDataRepository` directly.
