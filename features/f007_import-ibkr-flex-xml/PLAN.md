...
- **Resolve modal UX** — The spec constraint requires market data preview inline on the import page, but the plan didn't specify reusing the transaction form's preview mechanism. Post-implementation fix: Tasks A–E in NOTES.md add broker symbol to skipped reason, group same symbols, and reuse the preview panels in the modal.
- **Repeated unmapped symbols** — If multiple trades share the same broker symbol (e.g., 5 trades for "STHY"), the user must resolve one by one. Post-implementation fix: grouped unmapped symbols section with one resolve button per unique symbol.
- **Date format mismatch** — Sample XML was generated with `YYYY-MM-DD` dates but real IBKR uses `YYYYMMDD` without dashes, causing all imported dates to be `0001-01-01`. Post-implementation fix: added `parseDate` helper that handles both formats, updated sample XML and all tests to `YYYYMMDD`.
