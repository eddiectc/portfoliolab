...
## R1-2 Implementation Notes (2026-09-04)
- **Deviation (precision):** the plan did not specify rounding. govalues `Quo`/`Mul` preserve full scale, so proportional attributions with repeating ratios (e.g. 18/666) produced scale-19 values like `−1889.999999999999998`. Added `roundMoney` (2 dp) applied at each proportional multiplication: matcher `costBasisConsumed` + `sellProceedsPortion`, open-position cost portion. Money is stored/displayed at 2 dp, so per-portion rounding keeps sums, averages, and P&L% exact (VWRP: 712.80/1890.00 → Pct `37.71428571428571429`, scale 17, pinned in test).
- **Deviation (sign robustness):** `buildOpenPosition` normalizes each lot's delta by lot type (buy +, sell −) in the running-quantity walk. Some test fixtures (service_test helpers) store sell quantities as positive; the walk must be correct under either convention. `buildClosedPosition` and the matcher already use `Abs()`.
- **Bug fix (pre-existing, exposed by R1-2):** `MatchSellLotsAgainstBuys` had no per-symbol filter — a sell of symbol A could consume buy lots of symbol B. Harmless in the old model (matching only fed per-lot P&L) but corrupts cost attribution now that `remaining` drives open-position cost. Added symbol filter; mixed-symbol test pins it.
- Persistence audit: `positions` table has no unique constraint on (account, symbol, is_closed); repo `Recalculate` is delete-all + insert-all per account, so multiple closed rows per symbol (one per sell lot) persist cleanly. Service summary sums row-level `RealizedPnlBase` — row sum equals total realized P&L (each sale's proceeds attributed to exactly one closed row).
- Short open-date edge: buy-then-oversell (running never lands exactly on zero) falls back to the first lot's date per the plan's literal rule; only pure short-first sequences are exercised by tests.
