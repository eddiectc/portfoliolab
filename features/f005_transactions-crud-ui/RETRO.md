...
- [ ] Add unit tests for `Service.ListWithAccount` covering single-bound date filter synthesis, empty results, and pagination
- [ ] Add unit tests for `repository.ListWithAccount` covering all filter combinations and edge cases
- [ ] Update NOTES.md to remove the "No clear filters button" known limitation (it's implemented)
