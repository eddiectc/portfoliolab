...
func TestCapturePreCashFlowSnaps_NoCashFlows(t *testing.T) {
	txns := []transaction.Transaction{
		eqTxn(1, testTime(2024, 1, 15), "buy", "AAPL", "USD", 1000, 15000, -1500000),
		eqTxn(1, testTime(2024, 2, 15), "sell", "AAPL", "USD", -500, 17000, 850000),
	}

	quantities := map[string]decimal.Decimal{"AAPL": decimal.MustNew(500, 2)}
	cashBalance := map[string]decimal.Decimal{"USD": decimal.MustNew(-650000, 2)}
	positionCurrency := map[string]string{"AAPL": "USD"}

	snaps := capturePreCashFlowSnaps(testTime(2024, 1, 15), txns, quantities, cashBalance, positionCurrency)
	if len(snaps) != 0 {
		t.Errorf("expected 0 pre-cash-flow snapshots, got %d", len(snaps))
	}
}

func TestCapturePreCashFlowSnaps_SingleDeposit(t *testing.T) {
	txns := []transaction.Transaction{
		eqTxn(1, testTime(2024, 1, 15), "deposit", "$CASH-USD", "USD", 0, 0, 1000000),
	}

	// Final state after all transactions on Jan 15
	quantities := map[string]decimal.Decimal{} // no positions
	cashBalance := map[string]decimal.Decimal{"USD": decimal.MustNew(1000000, 2)}
	positionCurrency := map[string]string{}

	snaps := capturePreCashFlowSnaps(testTime(2024, 1, 15), txns, quantities, cashBalance, positionCurrency)
	if len(snaps) != 1 {
		t.Fatalf("expected 1 pre-cash-flow snapshot, got %d", len(snaps))
	}

	// Pre-cash-flow: no positions, cash = 0 (before the deposit)
	if len(snaps[0].positions) != 0 {
		t.Errorf("pre-cash-flow positions: expected 0, got %d", len(snaps[0].positions))
	}
	if !snaps[0].cashBalance["USD"].IsZero() {
		t.Errorf("pre-cash-flow cash: expected 0, got %s", snaps[0].cashBalance["USD"].String())
	}
}

func TestCapturePreCashFlowSnaps_DepositAfterBuy(t *testing.T) {
	txns := []transaction.Transaction{
		// Jan 15: buy 10 AAPL at $150
		eqTxn(1, testTime(2024, 1, 15), "buy", "AAPL", "USD", 1000, 15000, -1500000),
		// Jan 15: deposit $10,000 (same day, after buy)
		eqTxn(1, testTime(2024, 1, 15), "deposit", "$CASH-USD", "USD", 0, 0, 1000000),
	}

	// Final state after all transactions on Jan 15
	quantities := map[string]decimal.Decimal{"AAPL": decimal.MustNew(1000, 2)}
	cashBalance := map[string]decimal.Decimal{"USD": decimal.MustNew(-500000, 2)} // -1500000 + 1000000
	positionCurrency := map[string]string{"AAPL": "USD"}
	...
	// Pre-cash-flow: 10 AAPL, cash = -$1,500 (after buy, before deposit)
	if !snaps[0].positions["AAPL"].Equal(decimal.MustNew(1000, 2)) {
		t.Errorf("pre-cash-flow AAPL: got %s, want 1000", snaps[0].positions["AAPL"].String())
	}
	wantCash := decimal.MustNew(-1500000, 2)
	if !snaps[0].cashBalance["USD"].Equal(wantCash) {
		t.Errorf("pre-cash-flow cash: got %s, want %s", snaps[0].cashBalance["USD"].String(), wantCash.String())
	}
}
